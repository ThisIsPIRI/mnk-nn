...
from metrics import evaluate_player, human, play_game
...
def main():
	...
	with tf.Session() as sess:
		...
		if input("train the network?(y/n): ") == "y":
			runner.train(rules=rules, rewards=(1, 0, 0.3), batch_size=100, cycles=3000, stops=100, interactive=False, save_path=WEIGHT_PATH, session=sess)
		...
		#runner.train(rules=rules, rewards=(1, -1, 0.3), batch_size=2, cycles=2, stops=100, teachers=[(human, 1, 1)], interactive=True, save_path=WEIGHT_PATH, session=sess)
		#saver.save(sess, WEIGHT_PATH)
