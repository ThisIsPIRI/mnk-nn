...
def main():
	...
	human = Human()
	teachers = [(randai.play, 2, 0.5), (emacai.play, 2, 0.5), (None, 4, 0.5)]
	...
	with tf.Session() as sess:
		...
		if input("train the network?(y/n): ") == "y":
			runner.train(rules=rules, rewards=(1, -0.5, 0.3), batch_size=100, cycles=2000, stops=100, teachers=teachers, epsilon=0.05, interactive=False, save_path=WEIGHT_PATH, session=sess)
		...
		runner.train(rules=rules, rewards=(0.1, -1, 0), batch_size=1, cycles=2, stops=100, teachers=[(human.play, 1, 1)], interactive=True, save_path=WEIGHT_PATH, session=sess)
		saver.save(sess, WEIGHT_PATH)
		...
		#print(play_game(human.play, lambda g: runner.play(g, board=to_dense_input(g.array), session=sess, returnProbs=True), rules=rules, print_board=True))
